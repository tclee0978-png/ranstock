// app.js - 然然贏家 Pro 完整邏輯（中文註解 + 所有白皮書功能）
console.log('%c🚀 然然贏家 Pro 已啟動 • 煥然專屬高勝率系統', 'color:#c5a05b; font-size:18px; font-weight:bold');
...
function init() {
    ...
    spawnAIBubble();
    aiBubbleTimer = setInterval(spawnAIBubble, 8000);
...
function updateChartFooter() {
    ...
    if (!el) return;
    const now = new Date();
    el.textContent = `目前時間 ${now.getFullYear()}/${String(now.getMonth() + 1).padStart(2, "0")}/${String(now.getDate()).padStart(2, "0")} 盤後 • 歷史數據已載入`;
}
    ...
    stocks.forEach(stock => {
        ...
        item.innerHTML = `
            ...
            <div class="text-right">
                <div class="text-2xl font-bold ${stock.change > 0 ? "text-[#22c55e]" : "text-red-400"}">+${stock.change}%</div>
        ...
        options: {
            ...
            scales: {
                ...
                y1: {
                    position: "right",
                    grid: { display: false },
                    ticks: { color: "#22c55e", maxTicksLimit: 4 }
                },
                x: {
                    grid: { color: "#222" },
                    ticks: { color: "#c5a05b" }
...
function renderStockSelector() {
    ...
    mockStocks.forEach(stock => {
        ...
        opt.textContent = `${stock.symbol} ${stock.name} (+${stock.change}%)`;
...
function handleAIQuery() {
    ...
    setTimeout(() => {
        ...
        if (input.includes("追") || input.includes("噴") || input.includes("漲停")) {
            ...
            if (stock) {
                reply = stock.isFake
                    ? `${stock.name}：⚠️ 誘敵假突破，不要進！`
                    : `${stock.name}：${stock.grade}級 • ${stock.force}。符合紀律可觀察，停損 2% 必設。`;
            } else {
                reply = "進場前三問：量增嗎？站穩 5MA 嗎？是 S 級嗎？缺一不做。";
            }
